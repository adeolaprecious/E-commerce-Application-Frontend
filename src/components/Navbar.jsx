...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="shadow-md px-6 py-1 flex justify-between items-center text-amber-500" style={{ backgroundColor: '#fbf6ef', position: 'static' }}>
        <Link to="/home" className="flex items-center">
          <img src={logo1} alt="Logo" className="h-14 md:h-20" />
        </Link>
        <button className="md:hidden text-3xl focus:outline-none" onClick={() => setIsOpen(!isOpen)}>{isOpen ? '✖' : '☰'}</button>
        <ul className={`flex-col md:flex-row md:flex md:items-center md:space-x-8 absolute md:static bg-[#fbf6ef] left-0 w-full md:w-auto transition-all duration-300 ease-in-out ${isOpen ? 'top-[90px] opacity-100' : 'top-[-400px] opacity-0 md:opacity-100'}`} style={{ fontSize: '20px' }}>
